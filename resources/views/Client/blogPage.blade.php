        ...
        <div class="container mx-auto px-6 relative z-10 pt-20 text-center">
            <div class="max-w-3xl mx-auto animate-fade-up">
                <span class="text-secondary font-bold tracking-[0.3em] uppercase text-xs mb-4 block shadow-secondary/20">
                    @translate('Travel Journal')
                </span>
                <h1 class="text-4xl md:text-6xl font-extrabold text-white mb-6 font-serif">
                    @translate('Stories &') <span class="italic text-secondary">@translate('Inspiration')</span>
                </h1>
                <p class="text-lg text-white/80 leading-relaxed font-light mb-8">
                    @translate('Discover hidden gems, travel tips, and cultural insights from our adventures across Java and beyond.')
                </p>
            </div>
        </div>
    </section>
                ...
                <form action="{{ route('blogs.index') }}" method="GET" class="flex flex-col md:flex-row gap-4">
                    <div class="flex-grow relative">
                        ...
                        <input type="text" name="search" value="{{ request('search') }}"
                            placeholder="<?php echo \App\Helpers\TranslationHelper::translate('Search articles, guides...'); ?>" 
                    ...
                    <div class="w-full md:w-64 relative">
                        <select name="category" onchange="this.form.submit()"
                            ...
                            <option value="Tips" {{ request('category') == 'Tips' ? 'selected' : '' }}>@translate('Tips & Tricks')</option>
                            <option value="Destinations" {{ request('category') == 'Destinations' ? 'selected' : '' }}>@translate('Destinations')</option>
                            <option value="Culture" {{ request('category') == 'Culture' ? 'selected' : '' }}>@translate('Culture')</option>
                            <option value="Food" {{ request('category') == 'Food' ? 'selected' : '' }}>@translate('Food & Culinary')</option>
                        </select>
                    </div>
                    ...
                    <button type="submit" class="btn-premium px-8 py-4 rounded-xl font-bold shadow-lg flex items-center justify-center gap-2 group">
                        @translate('Search') <i class="fas fa-search group-hover:scale-110 transition-transform"></i>
                    </button>
                </form>
            </div>
        </div>
        ...
        @if(request('search') || request('category'))
            <div class="mb-8 animate-fade-up text-center relative z-10">
                <p class="text-gray-500">
                    @translate('Found') <span class="font-bold text-primary">{{ $blogs->total() }}</span> @translate('articles')
                    @if(request('search')) @translate('for') "<span class="text-primary">{{ request('search') }}</span>" @endif
                    @if(request('category')) @translate('in') <span class="text-primary">{{ request('category') }}</span> @endif
                </p>
                <a href="{{ route('blogs.index') }}" class="text-sm text-secondary font-bold hover:underline mt-2 inline-block">@translate('Clear Filters')</a>
            </div>
        @endif
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 relative z-10">
                ...
                <article class="bg-white rounded-[2rem] shadow-lg shadow-blue-900/5 border border-white/60 overflow-hidden group hover:shadow-xl hover:border-secondary/30 transition-all duration-300 animate-fade-up h-full flex flex-col">
                    ...
                    <div class="p-8 flex flex-col flex-grow">
                        <div class="flex items-center gap-4 text-xs text-gray-400 font-bold uppercase tracking-widest mb-4">
                            ...
                            <span class="flex items-center gap-1.5"><i class="far fa-clock"></i> {{ $blog->time_read ?? '5' }} @translate('min read')</span>
                        </div>
                        ...
                        <p class="text-gray-500 text-sm leading-relaxed mb-6 line-clamp-3 flex-grow">
                            {{-- Kita bungkus dulu contentnya dengan strip_tags, lalu translate --}}
                        ...
                        <div class="pt-6 border-t border-gray-100 mt-auto">
                            <a href="{{ route('blogs.detail', $blog->title) }}" class="inline-flex items-center text-sm font-bold text-primary group-hover:translate-x-2 transition-transform hover:text-secondary">
                                @translate('Read Full Story') <i class="fas fa-arrow-right ml-2 text-xs text-secondary"></i>
                            </a>
                        </div>
                    </div>
                </article>
            @empty
                <div class="col-span-full py-20 text-center animate-fade-up">
                    ...
                    <h3 class="text-2xl font-serif font-bold text-gray-800 mb-2">@translate('No articles found')</h3>
                    <p class="text-gray-500 max-w-md mx-auto mb-8">@translate("We couldn't find any articles matching your criteria. Try different keywords or browse all categories.")</p>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-10">
            <div class="lg:col-span-2">
                <div class="bg-white p-8 rounded-[2rem] shadow-lg shadow-blue-900/5 border border-white/60 animate-fade-up">
                    <h3 class="font-serif font-bold text-xl text-primary mb-6 pb-2 border-b border-gray-100">@translate('Latest Updates')</h3>
            ...
            <div class="lg:col-span-1">
                <div class="bg-primary rounded-[2rem] p-10 text-center text-white relative overflow-hidden animate-fade-up shadow-2xl h-full flex flex-col justify-center">
                    ...
                    <div class="relative z-10">
                        ...
                        <h2 class="text-3xl font-serif font-bold mb-4">@translate('Inspired to Travel?')</h2>
                        <p class="text-white/80 mb-8 font-light">
                            @translate('Start planning your dream trip with us today. We create custom itineraries tailored to your preferences.')
                        </p>
