...
@section('content')
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
                        ...
                        <div class="bg-white rounded-2xl shadow-lg overflow-hidden transition-all duration-300 hover:shadow-xl hover:-translate-y-2 flex flex-col h-[350px] animate-fade-up" style="transition-delay: 0.1s">
                            ...
                            <div class="p-6 flex flex-col flex-grow">
                                ...
                                <div class="flex items-center text-dark mb-4">
                                    ...
                                    <span class="text-sm">{{ $destination->place }}</span>
                                </div>
                                ...
                                <div class="flex flex-wrap gap-2 mb-5">
                                    @foreach(explode(',', $destination->activities) as $activity)
                                        @if(trim($activity))
                                            <span class="bg-light/70 text-primary px-3 py-1 rounded-full text-xs font-medium border border-primary/30">
                                                {{ trim($activity) }}
                                            </span>
                                        @endif
                                    @endforeach
                                ...
                                <div class="mt-auto">
                                    <a href="{{ route('destination.show', $destination->slug) }}"
                                       class="w-full text-light py-3 rounded-xl font-semibold transition-all duration-300 group-hover:scale-105 flex items-center justify-center bg-primary">
                                        <span class="text-white">Book Now</span>
                                        <i class="fas fa-arrow-right mt-0.5 ml-2 text-sm text-white group-hover:translate-x-1 transition-transform"></i>
                                    </a>
                                </div>
                            </div>
                        </div>
                    ...
                    <div class="bg-white rounded-2xl shadow-lg overflow-hidden transition-all duration-300 hover:shadow-xl hover:-translate-y-2 flex flex-col h-[350px] cursor-pointer group animate-fade-up" style="transition-delay: 0.4s"
                        ...
                        <div class="p-6 flex flex-col flex-grow">
                            ...
                            <div class="mt-auto">
                                <button class="w-full text-light py-3 rounded-xl font-semibold transition-all duration-300 hover:bg-warning group-hover:scale-105 flex items-center justify-center" style="background-color: #ffbc4c;">
                                    <span class="text-white">Customize Now</span>
                                    <i class="fas fa-arrow-right mt-0.5 ml-2 text-sm text-white group-hover:translate-x-1 transition-transform"></i>
                                </button>
                            </div>
                        </div>
    ...
    <div class="mb-8 text-center">
        <form method="GET" action="{{ route('blog.index') }}" class="inline-flex flex-wrap gap-2 justify-center">
            <!-- All Button -->
            <a href="{{ route('blog.index') }}"
               class="px-4 py-2 rounded-full text-sm font-medium 
                    {{ !request('type') ? 'bg-blue-600 text-white' : 'bg-gray-200 text-gray-800 hover:bg-gray-300' }}">
                All
            </a>
            ...
            @foreach($types as $type)
                <button type="submit"
                        ...
                        value="{{ $type }}"
                        class="px-4 py-2 rounded-full text-sm font-medium
                            {{ request('type') == $type ? 'bg-blue-600 text-white' : 'bg-gray-200 text-gray-800 hover:bg-gray-300' }}">
                    {{ $type }}
                </button>
            @endforeach
        </form>
